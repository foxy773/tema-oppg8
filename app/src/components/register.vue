...
export default {
  ...
  methods: {
    registerVisible(e) {
      ...
      if (e.target.className === background) {												// boolean in store too hide the register...
        this.$store.dispatch("switchRegisterVisible");									//	component if clicked
      }
    },
    ...
    async registerUser() {
      if (this.registerData.password === this.registerData.confirmedPassword) {	//	If both password fields are the same
        try {																							// use firebase function to create a new
          const auth = getAuth();																// user with the selected email and password
          const data = await createUserWithEmailAndPassword(							// and then proceed to send the user to his/hers
            auth,																						// personal profile page
            this.registerData.email,
            this.registerData.password
          );
          this.$store.dispatch("switchRegisterVisible");
          this.$router.push({ name: "profile" });
          this.registerUserSanity(data);														// If Firebase does not see anything wrong with
        } catch (err) {																				// Selected password or email then we perceed too
          console.log("something wrong with the register");								// create the database for the user
          console.log(err);
        }
      } else {
    ...
    async registerUserSanity(data) {															//	Sets up a template for the new user with a
      try {																								// Credits, wesite role, etc.
        await sanity
          .create({
            _type: "users",
            uid: data.user.uid,
